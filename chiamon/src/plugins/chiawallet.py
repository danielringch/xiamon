...
class Chiawallet(Plugin):
    def __init__(self, config, scheduler, outputs):
        ...
        self.__host, _ = config_data.get_value_or_default('127.0.0.1:9256', 'host')
        ...
        self.__rpc = Chiarpc(self.__host, config_data.data['cert'], config_data.data['key'],
            super(Chiawallet, self), self.__mute_interval)

        self.__wallet_unsynced_alert = Alert(super(Chiawallet, self), self.__mute_interval)

        self.__wallet_id, _ = config_data.get_value_or_default(1, 'wallet_id')
        self.__balance = None

        scheduler.add_job('chiawallet-check' ,self.check, config_data.get_value_or_default('0 0 * * *', 'check_interval')[0])
        scheduler.add_job('chiawallet-summary', self.summary, config_data.get_value_or_default('0 * * * *', 'summary_interval')[0])

    async def check(self):
        await self.send(Plugin.Channel.debug, f'Checking wallet {self.__wallet_id} on {self.__host} .')
